...
from industrial_rca.data.telemetry_generator import TelemetryDataset, generate_high_frequency_vibration
...
# ══════════════════════════════════════════════════════════════════════
# FASTAPI HARDWARE-IN-THE-LOOP (HIL) REST INGESTION ENDPOINT
# ══════════════════════════════════════════════════════════════════════
...
def start_fastapi_background_daemon(host: str = "0.0.0.0", port: int = 8000):
    ...
    try:
        ...
        # Port already bound (server already active)
        return
...
# Automatically launch API server in background
...
with st.sidebar:
    ...
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.markdown('<p style="font-family:var(--font-mono); font-size:0.65rem; color:#64748B; letter-spacing:0.08em; text-transform:uppercase; margin-bottom:8px;">📡 Edge HIL Middleware</p>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:rgba(15,23,42,0.8); border:1px solid rgba(0,212,170,0.2); border-radius:8px; padding:10px 12px; margin-bottom:8px;">
        <div style="display:flex; align-items:center; justify-content:space-between;">
            <span style="font-family:var(--font-mono); font-size:0.75rem; color:#E2E8F0;">REST API (Port 8000)</span>
            <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#00D4AA; box-shadow:0 0 6px #00D4AA;"></span>
        </div>
        <div style="font-family:var(--font-mono); font-size:0.65rem; color:#8892B0; margin-top:4px;">
            Endpoint: <span style="color:#60A5FA;">/api/v1/telemetry/incident</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if LATEST_HIL_INCIDENT.get("has_incident"):
        ...
        st.markdown(f"""
        <div style="background:rgba(255,75,75,0.15); border:1px solid rgba(255,75,75,0.4); border-radius:8px; padding:10px 12px; margin-bottom:8px;">
            <div style="font-family:var(--font-mono); font-size:0.72rem; font-weight:700; color:#FF4B4B;">🚨 LIVE HIL TRIP DETECTED</div>
            <div style="font-family:var(--font-mono); font-size:0.68rem; color:#E2E8F0; margin-top:3px;">Asset: {inc['asset_id']} | Code: {inc['fault_code']}</div>
            <div style="font-family:var(--font-mono); font-size:0.62rem; color:#94A3B8;">{inc['fault_description']}</div>
        </div>
        """, unsafe_allow_html=True)
